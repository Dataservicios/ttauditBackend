...
Route::group(['before'=>'auth'], function(){
    ...
    Route::get('reportBayer/{company_id?}/{url?}', ['as' => 'reportBayer', 'uses' => 'ReportBayerController@reportHome']);
...
Route::post('JsonInsertAuditPollsProduct', ['as' =>'JsonInsertAuditPollsProduct', function(){
    ...
    if ($consultaNum[0]->numero==0)
    {
        ...
        $auditor = $consultaAuditor[0]->auditor."(".$consultaAuditor[0]->user_id.")";
        $valoresCampaigne[33] = array('stock' => 445,'premio' => 446,'recomendo' =>443,'cerrado' =>441);
        $valoresCampaigne[35] = array('stock' => 485,'premio' => 486,'recomendo' =>483,'cerrado' =>481);
        $valoresCampaigne[39] = array('stock' => 522,'premio' => 523,'recomendo' =>520,'cerrado' =>518);
        $valoresCampaigne[41] = array('stock' => 560,'premio' => 561,'recomendo' =>558,'cerrado' =>556);
        $valoresCampaigne[44] = array('stock' => 577,'premio' => 578,'recomendo' =>575,'cerrado' =>573);
        $valoresCampaigne[60] = array('stock' => 851,'premio' => 852,'recomendo' =>849,'cerrado' =>847);
        $valoresCampaigne[65] = array('stock' => 943,'premio' => 944,'recomendo' =>941,'cerrado' =>939);
        $valoresCampaigne[70] = array('stock' => 1023,'premio' => 1024,'recomendo' =>1021,'cerrado' =>1019);
        ...
        if ($status['status']==0){
            ...
            {
                $sqlcoord="SELECT  a.cadenaRuc,a.type,a.fullname,a.address,a.district,a.ejecutivo,a.ubigeo FROM stores a WHERE a.id = '".$store_id['store_id']."'";
...
Route::post('JsonInsertAuditPolls', ['as' =>'JsonInsertAuditPolls', function(){
    ...
    if ($consultaNum[0]->numero==0){
        ...
        $valoresCampaigne[40] = array('cobro' => 548,'NoTransaccion' => 545,'externo' =>529,'interno' => 530, 'cerrado' =>528);
        $valoresCampaigne[45] = array('cobro' => 602,'NoTransaccion' => 599,'externo' =>583,'interno' => 584, 'cerrado' =>582);
        $valoresCampaigne[49] = array('cobro' => 700,'NoTransaccion' => 697,'externo' =>681,'interno' => 682, 'cerrado' =>680);
        $valoresCampaigne[51] = array('cobro' => 739,'NoTransaccion' => 736,'externo' =>720,'interno' => 721, 'cerrado' =>719);
        $valoresCampaigne[57] = array('cobro' => 822,'NoTransaccion' => 819,'externo' =>803,'interno' => 804, 'cerrado' =>802);
        $valoresCampaigne[64] = array('cobro' => 924,'NoTransaccion' => 921,'externo' =>905,'interno' => 906, 'cerrado' =>904);
        $valoresCampaigne[69] = array('cobro' => 1004,'NoTransaccion' => 1001,'externo' =>985,'interno' => 986, 'cerrado' =>984);
        ...
        $sqlcoord="SELECT  a.cadenaRuc,a.type,a.ejecutivo,a.coordinador,a.gerzonal,a.fullname, a.codclient,a.address,a.district,a.ubigeo FROM stores a WHERE a.id = '".$store_id['store_id']."'";
        $consulEmail = DB::select($sqlcoord);
        ...
        if (($sino['sino']==1) and ($options['options']<>1) and ($limits['limits']<>1) and ($media['media']<>1) and ($coment['coment']<>1) and ($coment_options['coment_options']<>1)){
            ...
            {
                $textoEmail = "ALERTA: ".$consulta2[0]->fullname." Agente cobro fuera del voucher. DIR: ".$consulEmail[0]->codclient;
                $textoContent = $textoEmail;
                $nombreEnvio = $consulEmail[0]->coordinador;
        ...
        if (($sino['sino']<>1) and ($options['options']==1) and ($limits['limits']<>1) and ($media['media']<>1) and ($coment['coment']<>1) and ($coment_options['coment_options']==1)){
            ...
            if ($poll_id['poll_id']==$valoresCampaigne[$company_id]['NoTransaccion'])
            {
                ...
                $textoEmail = "ALERTA: ".$consulta2[0]->fullname." Dependiente NO acepto Trans. DIR: ".$consulEmail[0]->codclient;
                $textoContent = $textoEmail;
                $nombreEnvio = $consulEmail[0]->coordinador;
        ...
        if (($sino['sino']==1) and ($options['options']==1) and ($limits['limits']<>1) and ($media['media']==1) and ($coment['coment']<>1) and ($coment_options['coment_options']<>1)){
            ...
            $sqlcoord="SELECT  a.cadenaRuc,a.type,b.fullname as ejecutivo,b.email,a.coordinador,a.fullname, a.codclient,a.address,a.district,a.ubigeo,a.id FROM stores a, users b WHERE a.id = '".$store_id['store_id']."' AND b.email = a.coordinador";
            $consulEmail = DB::select($sqlcoord);
            ...
            if ((($poll_id['poll_id']==$valoresCampaigne[$company_id]['externo']) or ($poll_id['poll_id']==$valoresCampaigne[$company_id]['interno'])) and ($result['result']==0))
            {
                if ($poll_id['poll_id']==$valoresCampaigne[$company_id]['externo']){
                    $textoEmail = "ALERTA: ".$consulta2[0]->fullname." Letrero Externo NO visible DIR: ".$consulEmail[0]->codclient;
                }
                if ($poll_id['poll_id']==$valoresCampaigne[$company_id]['interno']){
                    $textoEmail = "ALERTA: ".$consulta2[0]->fullname." Letrero Interno NO visible DIR: ".$consulEmail[0]->codclient;
                }
                $textoContent = $textoEmail;
                $nombreEnvio = $consulEmail[0]->coordinador;
                ...
                $envio = ['responsable' => 'Franco','email' =>'franbrsj@gmail.com', 'subject' =>$textoEmail];
                \Mail::send('emails.localCerrado', $datai, function($message) use ($envio){
                    $message->to($envio['email'], $envio['responsable'])->subject($envio['subject']);
                });
            ...
            $opciones = explode('|',$opcion['opcion']);
            //return \Response::json([ 'result'=> $opciones]);
                    ...
                    if (count($consulta1)>0){
                        ...
                        if (($poll_id['poll_id']==$valoresCampaigne[$company_id]['cerrado']) and ($result['result']==0))
                        {
                            ...
                            if ($poll_id['poll_id']==$valoresCampaigne[$company_id]['cerrado']){
                                $textoEmail = "ALERTA: ".$consulta2[0]->fullname." Agente Cerrado DIR: ".$consulEmail[0]->codclient;
                            }
                            ...
                            $emails[]=array('nombres'=>$consulEmail[0]->ejecutivo,'email'=>$consulEmail[0]->email);


                            $sqlejec1="SELECT  b.fullname,b.email,a.ejecutivo FROM stores a, users b WHERE a.id = '".$store_id['store_id']."' AND b.email = a.ejecutivo";
                            $consulEmail1 = DB::select($sqlejec1);

                            $emails[]=array('nombres'=>$consulEmail1[0]->fullname,'email'=>$consulEmail1[0]->email);
                            ...
                            if ($poll_id['poll_id']==$valoresCampaigne[$company_id]['cerrado']){
                                ...
                                if (count($emails)>0){$c=0;$ListEmails='';
                                    ...
                                    DB::insert("INSERT INTO alerts (store_id, user_id,titulo,motivo,company_id,emails,send, created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?)" , array($store_id['store_id'],$consultaAuditor[0]->user_id,$textoEmail,$motivo,$idcompany['idCompany'],$ListEmails,'1',$horaSistema,$horaSistema));
                                    $idPollDetail = DB::getPdo()->lastInsertId();
                                }
                            }
                        }
        ...
        if (($sw==1) and ($soloTtaudit==0))
        {
            if ($consulEmail[0]->ejecutivo<>'')
            {
                $emails[]=array('nombres'=>'Ejecutivo IBK','email'=>$consulEmail[0]->ejecutivo);

            }
            if ($consulEmail[0]->coordinador<>'')
            {
                $emails[]=array('nombres'=>'Coordinador IBK','email'=>$consulEmail[0]->coordinador);

            }
            if ($consulEmail[0]->gerzonal<>'')
            {
                $emails[]=array('nombres'=>'Ejecutivo Zonal','email'=>$consulEmail[0]->gerzonal);
            }
            $emails[]=array('nombres'=>'Augusto','email'=>'aguerra@ttaudit.com');
            $emails[]=array('nombres'=>'Raul','email'=>'rpulido@ttaudit.com');
            $emails[]=array('nombres'=>'Daniela','email'=>'dolaguibel@ttaudit.com');
            ...
            if (($poll_id['poll_id']==$valoresCampaigne[$company_id]['NoTransaccion']) or ($poll_id['poll_id']==$valoresCampaigne[$company_id]['cobro'])){
                $emails[]=array('nombres'=>'Karina','email'=>'Kpalominof@intercorp.com.pe');
            }
            if (count($emails)>0){$c=0;$ListEmails='';
                foreach ($emails as $email) {
...
//Alicorp Mayoristas
    ...
    $sql1="SELECT id FROM stores where id in (58868,58872)";
...
Route::post('getPointStoresForCompanyDepartament', ['as' =>'getPointStoresForCompanyDepartament', function(){
    /*$company_id= Input::only('company_id');*/
...
FROM
  `companies`
  RIGHT OUTER JOIN `company_stores` ON (`companies`.`id` = `company_stores`.`company_id`)
...
WHERE
  `stores`.`test` = 0 AND
  `stores`.`ubigeo` = '".$departament['departament']."' AND
  `company_stores`.`ruteado` = 0";
}else{
    $sql1 = "SELECT
  `stores`.`id`,
  `stores`.`cadenaRuc`,
