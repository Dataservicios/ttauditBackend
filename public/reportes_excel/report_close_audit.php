...
$query_detalle_puntos = "SELECT `principal`.`store_id`,`principal`.`Comercio`,`principal`.`id`,`principal`.`Ruta`,`principal`.`user_id`,`principal`.`Auditor`,
`principal`.`company_id`,`principal`.`campaigne`,`principal`.`customer_id`,`principal`.`Cliente`,`principal`.`fecha`,`principal`.`hora`,
`lat_close`,`long_close`,`lat_open`,`long_open`,`time_open`,date_format(`time_open`, '%d/%m/%Y') AS `fecha_open`,date_format(`time_open`, '%H:%i:%s') AS `hora_open`,`time_close`,date_format(`time_close`, '%d/%m/%Y') AS `fecha_close`,date_format(`time_close`, '%H:%i:%s') AS `hora_close`,`created_at` as `created_control`,`updated_at` as `updated_control`
...
ORDER BY `road_details`.`updated_at` DESC)
union all
(SELECT
  `a`.`store_id`,
`stores`.`fullname` as `Comercio`,
...
WHERE
  `f`.`company_id` = 47 AND
  date_format(`a`.`created_at`, '%m/%Y') = date_format(now(), '%m/%Y')
GROUP BY
  `f`.`company_id`,
  `a`.`store_id`,
  date_format(`a`.`created_at`, '%d/%m/%Y')
ORDER BY `a`.`created_at` DESC)) as principal

LEFT OUTER JOIN `control_time` `TI` ON (`TI`.`store_id` = `principal`.`store_id` and `TI`.`company_id` = `principal`.`company_id` and `TI`.`user_id` = `principal`.`user_id`) group by `principal`.`store_id`";
...
$objPHPExcel->getActiveSheet()->setTitle('Resumen Puntos cerrados Mensual');
...
header('Content-Disposition: attachment;filename="REPORTE_PUNTOS_CERRADOS.xlsx"');
