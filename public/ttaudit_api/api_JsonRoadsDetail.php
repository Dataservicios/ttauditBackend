...
$statement=$pdo->prepare('SELECT
  ...
  `roads_resume`.`distributor`,
  `roads_resume`.`urbanization`,
  `roads_resume`.`type`,
  `roads_resume`.`store_type_id`,
  `roads_resume`.`rubro`,
  ...
  `roads_resume`.`owner`,
  `roads_resume`.`fnac`,
  `store_types`.`fullname`  as store_type_name
FROM roads_resume 
 LEFT OUTER JOIN `store_types` ON (`store_types`.`id` = `roads_resume`.`store_type_id`)
 where `roads_resume`.`company_id`=:company_id and `roads_resume`.`user_id`=:user_id and  `roads_resume`.`nivel` = 1');
