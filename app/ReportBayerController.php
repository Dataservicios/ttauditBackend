...
class ReportBayerController extends BaseController{
    ...
    public function reportHome($company_id="0",$url="0")
    {
        ...
        $customer =$this->customerRepo->find($campaigneDetail->customer_id);//dd($customer);
        $logo = $this->urlBase.$this->urlImageBase.$customer->corto.'/'.$customer->logo;
        $campaignesClient = $this->companyRepo->getCompaniesForClient($customer->id,1);//Para combo de cambio de campañas collection array de objetos Company
    ...
    public function auditReportAbstract($audit_id="0",$company_id="0",$subopcion="0")
    {
        ...
        $customer =$this->customerRepo->find($campaigneDetail->customer_id);//dd($customer);
        $logo = $this->urlBase.$this->urlImageBase.$customer->corto.'/'.$customer->logo;
        $campaignesClient = $this->companyRepo->getCompaniesForClient($customer->id,1);//Para combo de cambio de campañas
        ...
        if ($subopcion=='Moderno'){
            $ListHorizontales[] ="Mini Cadenas";$cat=4;
        }
        if ($subopcion=='Tradicional'){
            $ListHorizontales[] = "Detallista";$ListHorizontales[] ="Sub Distribuidor";$cat=5;
        }
        if ($subopcion=='0'){
            $ListHorizontales[] = "Detallista";$ListHorizontales[] ="Mini Cadenas";$ListHorizontales[] ="Sub Distribuidor";$cat=0;
        }
    ...
    public function homeComparationSellerCampaigns($company_id="0",$product_id="0",$cadena="0",$horizontal="0")
    {
        ...
        $getCompaignes = $this->companyRepo->getCompaniesForClient($customer->id);
    ...
    public function ajaxGetRecomendSalesForProduct()
    {
        ...
        $company = $this->companyRepo->find($company_id);
        $companies = $this->companyRepo->getCompaniesForClient($company->customer_id);
    ...
    public function ajaxGetRecomendSalesForSeller()
    {
        ...
        $company = $this->companyRepo->find($company_id);
        $companies = $this->companyRepo->getCompaniesForClient($company->customer_id);//dd($ejecutivo);
        $group_poll_id='';$c=0;
