...
BEGIN
    ...
    IF EXISTS (SELECT id FROM tempory_processes ) THEN
      ...
      IF  @status_ = 0 THEN
        ...
        UPDATE `tempory_processes` SET `status` = 1 , `processes` = 'sp_bayert_visibilidad',`updated_at` = @updated_at;
        ...
        IF _tipo = 0 THEN
          IF _pag = 1  THEN
            IF _company_id >= 195 THEN
              SET @publicity_id_1 = 561;
              SET @publicity_id_2 = 924;
              SET @publicity_id_3 = 565;
            ELSE
              SET @publicity_id_1 = 561;
              SET @publicity_id_2 = 563;
              SET @publicity_id_3 = 565;
            end if;
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_si_no_3_561 ( INDEX(store_id ) ) AS
              (
              select * from tmp_sino_pop where poll_id = @poll__id_3 and publicity_id=@publicity_id_1
              );
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_Base_561 ( INDEX(store_id ) ) AS
              (
              SELECT count(publicity_id) as total, store_id FROM publicity_store p where  company_id=_company_id and visit_id=0 and publicity_id=@publicity_id_1 group by publicity_id,store_id
              );
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_si_no_10_561 ( INDEX(store_id ) ) AS
              (
              select * from tmp_sino_pop where poll_id = @poll__id_10 and publicity_id=@publicity_id_1
              );
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_si_no_4_561 ( INDEX(store_id ) ) AS
              (
              select * from tmp_sino_pop where poll_id = @poll__id_4 and publicity_id=@publicity_id_1
              );
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_4_561 ( INDEX(store_id ) ) AS
              ...
              where  poll_id = @poll__id_4 and publicity_id=@publicity_id_1
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_si_no_5_561 ( INDEX(store_id ) ) AS
              (
              select * from tmp_sino_pop where poll_id = @poll__id_5 and publicity_id=@publicity_id_1
              );
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_5_561 ( INDEX(store_id ) ) AS
              ...
              select store_id,publicity_id, respuesta,
                     ...
                     otro
              from tmp_opciones_pop
              where  poll_id = @poll__id_5 and publicity_id=@publicity_id_1
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_si_no_6_561 ( INDEX(store_id ) ) AS
              (
              select * from tmp_sino_pop where poll_id = @poll__id_6 and publicity_id=@publicity_id_1
              );
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_6_561 ( INDEX(store_id ) ) AS
              ...
              select store_id,publicity_id, respuesta,
                     ...
                     otro
              from tmp_opciones_pop
              where  poll_id = @poll__id_6 and publicity_id=@publicity_id_1
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_si_no_3_563 ( INDEX(store_id ) ) AS
              (
              select * from tmp_sino_pop where poll_id = @poll__id_3 and publicity_id=@publicity_id_2
              );
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_Base_563 ( INDEX(store_id ) ) AS
              (
              SELECT count(publicity_id) as total, store_id FROM publicity_store p where  company_id=_company_id and visit_id=0 and publicity_id=@publicity_id_2 group by publicity_id,store_id
              );
            /*Fin especial Base Bayer*/
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_si_no_4_563 ( INDEX(store_id ) ) AS
              (
              select * from tmp_sino_pop where poll_id = @poll__id_4 and publicity_id=@publicity_id_2
              );
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_4_563 ( INDEX(store_id ) ) AS
              ...
              select store_id,publicity_id, respuesta,
                     ...
                     otro
              from tmp_opciones_pop
              where  poll_id = @poll__id_4 and publicity_id=@publicity_id_2
              group by store_id, respuesta, otro,publicity_id
              );
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_si_no_5_563 ( INDEX(store_id ) ) AS
              (
              select * from tmp_sino_pop where poll_id = @poll__id_5 and publicity_id=@publicity_id_2
              );
            ...
            IF _company_id >= 195 THEN
              ...
              CREATE TEMPORARY TABLE IF NOT EXISTS preg_5_563 ( INDEX(store_id ) ) AS
                (
                select store_id,publicity_id, respuesta,
                       sum(case when options = CONVERT(CONCAT(@poll__id_5 ,'b') using utf8) collate utf8_spanish_ci then 1 else 0 end) as a,
                       sum(case when options = CONVERT(CONCAT(@poll__id_5 ,'c') using utf8) collate utf8_spanish_ci then 1 else 0 end) as b,
                       sum(case when options = CONVERT(CONCAT(@poll__id_5 ,'g') using utf8) collate utf8_spanish_ci then 1 else 0 end) as c,
                       otro
                from tmp_opciones_pop
                where  poll_id = @poll__id_5 and publicity_id=@publicity_id_2
                group by store_id, respuesta,product_id, otro,publicity_id
                );
            else
              DROP TEMPORARY TABLE IF EXISTS preg_5_563;
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_si_no_6_563 ( INDEX(store_id ) ) AS
              (
              select * from tmp_sino_pop where poll_id = @poll__id_6 and publicity_id=@publicity_id_2
              );
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_6_563 ( INDEX(store_id ) ) AS
              ...
              select store_id,publicity_id, respuesta,
                     ...
                     otro
              from tmp_opciones_pop
              where  poll_id = @poll__id_6 and publicity_id=@publicity_id_2
              group by store_id, respuesta,product_id,foto, otro,publicity_id
              );
            --
            -- /*565  --------------------------------------------  Paneles*/
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_si_no_3_565 ( INDEX(store_id ) ) AS
              (
              select * from tmp_sino_pop where poll_id = @poll__id_3 and publicity_id=@publicity_id_3
              );
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_Base_565 ( INDEX(store_id ) ) AS
              (
              SELECT count(publicity_id) as total, store_id FROM publicity_store p where  company_id=_company_id and visit_id=0 and publicity_id=@publicity_id_3 group by publicity_id,store_id
              );
            /*Fin especial Base Bayer*/
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_si_no_10_565 ( INDEX(store_id ) ) AS
              (
              select * from tmp_sino_pop where poll_id = @poll__id_10 and publicity_id=@publicity_id_3
              );

            DROP TEMPORARY TABLE IF EXISTS preg_si_no_4_565;
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_si_no_4_565 ( INDEX(store_id ) ) AS
              (
              select * from tmp_sino_pop where poll_id = @poll__id_4 and publicity_id=@publicity_id_3
              );
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_4_565 ( INDEX(store_id ) ) AS
              ...
              select store_id,publicity_id, respuesta,
                     ...
                     otro
              from tmp_opciones_pop
              where  poll_id = @poll__id_4 and publicity_id=@publicity_id_3
              group by store_id, respuesta, otro,publicity_id
              );
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_si_no_5_565 ( INDEX(store_id ) ) AS
              (
              select * from tmp_sino_pop where poll_id = @poll__id_5 and publicity_id=@publicity_id_3
              );
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_5_565 ( INDEX(store_id ) ) AS
              ...
              select store_id,publicity_id, respuesta,
                     ...
                     otro
              from tmp_opciones_pop
              where  poll_id = @poll__id_5 and publicity_id=@publicity_id_3
              group by store_id, respuesta,product_id, otro,publicity_id
              );
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_si_no_6_565 ( INDEX(store_id ) ) AS
              (
              select * from tmp_sino_pop where poll_id = @poll__id_6 and publicity_id=@publicity_id_3
              );
            ...
            CREATE TEMPORARY TABLE IF NOT EXISTS preg_6_565 ( INDEX(store_id ) ) AS
              ...
              select store_id,publicity_id, respuesta,
                     ...
                     otro
              from tmp_opciones_pop
              where  poll_id = @poll__id_6 and publicity_id=@publicity_id_3
              group by store_id, respuesta,product_id,foto, otro,publicity_id
              );
            ...
            IF _company_id >= 195 THEN
              select
                  ...
                  -- (case when preg_si_no_5_563.Respuesta > 0 then 2 else '0' end) as '5_563_Respuesta',
                     ...
                     (case when preg_5_563.b > 0 then 2 else preg_5_563.b end) as '5_563_b',
                     (case when preg_5_563.c > 0 then 2 else preg_5_563.c end) as '5_563_c',
          ...
          IF _pag = 2 THEN
            IF _company_id >= 195 THEN
              SET @publicity_id_1 = 566;
              SET @publicity_id_2 = 567;
              SET @publicity_id_3 = 960;
              SET @publicity_id_4 = 996;
            ELSE
              SET @publicity_id_1 = 566;
              SET @publicity_id_2 = 567;
              SET @publicity_id_3 = 562;
            end if;
            ...
            IF _company_id >= 195 THEN
              ...
              CREATE TEMPORARY TABLE IF NOT EXISTS preg_5_562 ( INDEX(store_id ) ) AS
                ...
                select store_id,publicity_id, respuesta,
                       sum(case when options = CONVERT(CONCAT(@poll__id_5 ,'a') using utf8) collate utf8_spanish_ci then 1 else 0 end) as a,
                       sum(case when options = CONVERT(CONCAT(@poll__id_5 ,'b') using utf8) collate utf8_spanish_ci then 1 else 0 end) as b,
                       sum(case when options = CONVERT(CONCAT(@poll__id_5 ,'g') using utf8) collate utf8_spanish_ci then 1 else 0 end) as g,
                       otro
                from tmp_opciones_pop
                where  poll_id = @poll__id_5 and publicity_id=@publicity_id_3
                group by store_id, respuesta,product_id, otro,publicity_id
            ...
            IF _company_id >= 207 THEN
              /*996  ------------------------------ Pedestal*/

              DROP TEMPORARY TABLE IF EXISTS preg_si_no_3_996;
              CREATE TEMPORARY TABLE IF NOT EXISTS preg_si_no_3_996 ( INDEX(store_id ) ) AS
                (
                select * from tmp_sino_pop where poll_id = @poll__id_3 and publicity_id=@publicity_id_4
                );

              /*Especial Base Bayer*/
              DROP TEMPORARY TABLE IF EXISTS preg_Base_996;
              CREATE TEMPORARY TABLE IF NOT EXISTS preg_Base_996 ( INDEX(store_id ) ) AS
                (
                SELECT count(publicity_id) as total, store_id FROM publicity_store p where  company_id=_company_id and visit_id=0 and publicity_id=@publicity_id_4 group by publicity_id,store_id
                );
              /*Fin especial Base Bayer*/

              DROP TEMPORARY TABLE IF EXISTS preg_si_no_4_996;
              CREATE TEMPORARY TABLE IF NOT EXISTS preg_si_no_4_996 ( INDEX(store_id ) ) AS
                (
                select * from tmp_sino_pop where poll_id = @poll__id_4 and publicity_id=@publicity_id_4
                );
              DROP TEMPORARY TABLE IF EXISTS preg_4_996;
              CREATE TEMPORARY TABLE IF NOT EXISTS preg_4_996 ( INDEX(store_id ) ) AS
                (
                select store_id,publicity_id, respuesta,
                       sum(case when options = CONVERT(CONCAT(@poll__id_4 ,'a') using utf8) collate utf8_spanish_ci then 1 else 0 end) as a,
                       sum(case when options = CONVERT(CONCAT(@poll__id_4 ,'b') using utf8) collate utf8_spanish_ci then 1 else 0 end) as b,
                       otro
                from tmp_opciones_pop
                where  poll_id = @poll__id_4 and publicity_id=@publicity_id_4
                group by store_id, respuesta, otro,publicity_id
                );

              DROP TEMPORARY TABLE IF EXISTS preg_si_no_5_996;
              CREATE TEMPORARY TABLE IF NOT EXISTS preg_si_no_5_996 ( INDEX(store_id ) ) AS
                (
                select * from tmp_sino_pop where poll_id = @poll__id_5 and publicity_id=@publicity_id_4
                );
              DROP TEMPORARY TABLE IF EXISTS preg_5_996;
              CREATE TEMPORARY TABLE IF NOT EXISTS preg_5_996 ( INDEX(store_id ) ) AS
                (
                select store_id,publicity_id, respuesta,
                       sum(case when options = CONVERT(CONCAT(@poll__id_5 ,'a') using utf8) collate utf8_spanish_ci then 1 else 0 end) as a,
                       sum(case when options = CONVERT(CONCAT(@poll__id_5 ,'c') using utf8) collate utf8_spanish_ci then 1 else 0 end) as b,
                       ...
                       otro
                from tmp_opciones_pop
                where  poll_id = @poll__id_5 and publicity_id=@publicity_id_4
                group by store_id, respuesta,product_id, otro,publicity_id
                );

              DROP TEMPORARY TABLE IF EXISTS preg_si_no_6_996;
              CREATE TEMPORARY TABLE IF NOT EXISTS preg_si_no_6_996 ( INDEX(store_id ) ) AS
                (
                select * from tmp_sino_pop where poll_id = @poll__id_6 and publicity_id=@publicity_id_4
                );
              DROP TEMPORARY TABLE IF EXISTS preg_6_996;
              CREATE TEMPORARY TABLE IF NOT EXISTS preg_6_996 ( INDEX(store_id ) ) AS
                (
                select store_id,publicity_id, respuesta,
                       sum(case when options = CONVERT(CONCAT(@poll__id_6 ,'a') using utf8) collate utf8_spanish_ci then 1 else 0 end) as a,
                       sum(case when options = CONVERT(CONCAT(@poll__id_6 ,'b') using utf8) collate utf8_spanish_ci then 1 else 0 end) as b,
                       ...
                       otro
                from tmp_opciones_pop
                where  poll_id = @poll__id_6 and publicity_id=@publicity_id_4
                group by store_id, respuesta,product_id,foto, otro,publicity_id
                );
            end if;
            ...
            IF _company_id >= 195 THEN
              select
                  ...
                  -- (case when preg_si_no_5_562.Respuesta > 0 then 2 else '0' end) as '5_562_Respuesta',
                     ...
                     (case when preg_5_562.b > 0 then 2 else preg_5_562.b end) as '5_562_b',
                     (case when preg_5_562.g > 0 then 2 else preg_5_562.g end) as '5_562_g',
            ...
            IF _company_id >= 207 THEN
              select
                  ...
                  --
                  -- (case when preg_si_no_3_567.Respuesta > 0 then 2 else '0' end) as '3_567_Respuesta',
                     if(preg_si_no_3_996.Respuesta is null , '' , if (preg_si_no_3_996.Respuesta > 0, 2 , 0)) as '3_996_Respuesta',
                     preg_si_no_3_996.Foto as '3_996_Foto',
                     (case when preg_Base_996.total  is null then 0 else preg_Base_996.total end) as  'Base_996_total',
                  -- (case when preg_si_no_4_996.Respuesta > 0 then 2 else '0' end) as '4_996_Respuesta',
                     if(preg_si_no_4_996.Respuesta is null , '' , if (preg_si_no_4_996.Respuesta > 0, 2 , 0)) as '4_996_Respuesta',
                     (case when preg_4_996.a > 0 then 2 else preg_4_996.a end) as '4_996_a',
                     (case when preg_4_996.b > 0 then 2 else preg_4_996.b end) as '4_996_b',
                     preg_4_996.otro as '4_996_Comentario',

                  -- (case when preg_si_no_5_996.Respuesta > 0 then 2 else '0' end) as '5_996_Respuesta',
                     if(preg_si_no_5_996.Respuesta is null , '' , if (preg_si_no_5_996.Respuesta > 0, 2 , 0)) as '5_996_Respuesta',
                     (case when preg_5_996.a > 0 then 2 else preg_5_996.a end) as '5_996_a',
                     (case when preg_5_996.b > 0 then 2 else preg_5_996.b end) as '5_996_b',
                     (case when preg_5_996.c > 0 then 2 else preg_5_996.c end) as '5_996_c',
                     preg_5_996.otro as '5_996_Comentario',

                  -- (case when preg_si_no_6_996.Respuesta > 0 then 2 else '0' end) as '6_996_Respuesta',
                     if(preg_si_no_6_996.Respuesta is null , '' , if (preg_si_no_6_996.Respuesta > 0, 2 , 0)) as '6_996_Respuesta',
                     (case when preg_6_996.a > 0 then 2 else preg_6_996.a end) as '6_996_a',
                     (case when preg_6_996.b > 0 then 2 else preg_6_996.b end) as '6_996_b',
                     (case when preg_6_996.c > 0 then 2 else preg_6_996.c end) as '6_996_c',
                     preg_si_no_6_996.comentario as '6_996_Comentario',
                     preg_si_no_6_996.Foto as '6_996_Foto'
                  --
              from tmp_comercios_pop
                     ...
                     left outer join preg_6_567 on (tmp_comercios_pop.store_id = preg_6_567.store_id )
                       ...
                       -- /*996*/
                       --
                     left outer join preg_si_no_3_996 on (tmp_comercios_pop.store_id = preg_si_no_3_996.store_id )

                     left outer join preg_Base_996 on (tmp_comercios_pop.store_id = preg_Base_996.store_id )

                     left outer join preg_si_no_4_996 on (tmp_comercios_pop.store_id = preg_si_no_4_996.store_id )
                     left outer join preg_4_996 on (tmp_comercios_pop.store_id = preg_4_996.store_id )

                     left outer join preg_si_no_5_996 on (tmp_comercios_pop.store_id = preg_si_no_5_996.store_id )
                     left outer join preg_5_996 on (tmp_comercios_pop.store_id = preg_5_996.store_id )

                     left outer join preg_si_no_6_996 on (tmp_comercios_pop.store_id = preg_si_no_6_996.store_id )
                     left outer join preg_6_996 on (tmp_comercios_pop.store_id = preg_6_996.store_id );
            end if;
        ...
        /*fin*/
        SET @updated_at = (select DATE_SUB(NOW(),INTERVAL 5 HOUR));
        INSERT INTO `log_processes`(`processes`,`status`,`company_id`,`user_id`,`created_at`,`updated_at`) VALUES ('sp_bayert_visibilidad',1, _company_id , _user_id,@created_at,@updated_at );
        UPDATE `tempory_processes` SET `status` = 0,  `processes` = 'sp_bayert_visibilidad',`updated_at` = @updated_at;

      END IF;
    ELSE
      SET @updated_at = (select DATE_SUB(NOW(),INTERVAL 5 HOUR));
      INSERT INTO `tempory_processes`(`processes`,`status`,`company_id`,`user_id`,`created_at`,`updated_at`) VALUES ('sp_bayert_visibilidad',0, _company_id , 0,@created_at,@updated_at);
    END IF;
  END