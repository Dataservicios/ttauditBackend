...
class AlertsController extends BaseController
{
    ...
    public function getLastAlerts($customer_id)
    {
        ...
        $alerts = $this->alertRepo->getLatestAlerts(50,$customer_id);
        if (count($alerts)>0){
            foreach ($alerts as $alert) {
                $objStore = $this->storeRepo->find($alert->store_id);
                if (is_object($objStore)){
                    $ciudad = $objStore->ubigeo;
                    $objUser = $this->userRepo->getUserForEmail($objStore->ejecutivo,$customer_id);//dd($objStore->ejecutivo,$customer_id,$objUser);
                ...
                $valores[] = array('id'=>$alert->id,'store_id'=>$store_id,'punto'=>$punto,'titulo'=>$alert->titulo,'motivo' => $alert->motivo,'ciudad'=>$ciudad,'ejecutivo'=>$ejecutivo,'comentado'=>$comentado,'fecha'=>$alert->updated_at);
            }
        }else{
            $valores =[];
        }
        ...
        return View::make('report/interbank/listAlerts', compact('typeUser','titulo','valores','logo','menus','campaignes'));
    }
    ...
    public function insertComment()
    {
        ...
        $objUserEjecutivo = $this->userRepo->find($user_id);
        $objAlert= $this->alertRepo->find($alert_id);
        $objUserAuditor = $this->userRepo->find($objAlert->user_id);
        $datoAuditor = $objUserAuditor->fullname."(".$objUserAuditor->id.")";
        $objStore = $this->storeRepo->find($objAlert->store_id);
        $store_id = $objStore->id;
        $textoContent = "Respuesta Pdv: ".$objStore->fullname." (".$objStore->id.") Ejecutivo: ".$objUserEjecutivo->fullname;
        $comentario = $message;
        $company_id = $objAlert->company_id;
        ...
        $district = $objStore->district;
        $fechaHoraEnvio = $horaSistema;
        $foto = "";$dir=$objStore->codclient;$agente=$objStore->fullname;$tipo_bodega=$alert_id;
        foreach ($questions as $question)
        {
            if ($question['company_id']=="response_alerts")
            {
                $motivo = "Respuesta alerta (".$objAlert->id."): ".$objAlert->titulo;
                ...
                for($i=0;$i<count($mascaras);$i++) {
                    ...
                    $this->sendEmails($store_id,$textoContent,$motivo,$datoAuditor,$comentario,$cliente,$tipo_bodega,$agente,$dir,$address,$district,$foto,$fechaHoraEnvio,$emails,"Alerts");
                }
