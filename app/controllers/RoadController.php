...
use Auditor\Managers\RoadManager;
...
class RoadController extends BaseController{
    ...
    public function edit($id)
    {
        ...
        return View::make('roads/editRoad',compact('road','urlBase'));
    }

    public function updateRoad($id)
    {
        $road = $this->roadRepo->find($id);
        //dd(Input::all());
        $manager = new RoadManager($road, Input::all());
        $manager->save();
    ...
    public function saveRouteVisits(){
        ...
        if ($lastInsert>0) {
            ...
            for ($i = 0; $i < count($id_store['id_store']); ++$i) {
                ...
                $num_visitas = round($objStore->visits);
                $regVisitStore = $objVisitStore->where('company_id',$valores[1])->where('store_id',$objStore->id)->orderBy('visit_id','DESC')->first();
                if (count($regVisitStore)==0)
                {
                    $visit_id_now = "0";
                    $visit_id_new = 1;
                }else{
                    $visit_id_now = $regVisitStore->visit_id;
                    $visit_id_new = $regVisitStore->visit_id+1;
                }
