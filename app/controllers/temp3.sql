CREATE  PROCEDURE `sp_alicorp_regular_sod_v6`(IN _company_id INT, IN _desde INT, IN _hasta INT,IN _user_id INT)
BEGIN
...
UPDATE `tempory_processes` SET `status` = 1 ,`processes` = 'sp_alicorp_regular_sod_v6';
...
INSERT INTO `log_processes`(`processes`,`status`,`company_id`,`user_id`,`created_at`,`updated_at`) VALUES ('sp_alicorp_regular_sod_v6',1, _company_id , _user_id,@created_at,@updated_at);
UPDATE `tempory_processes` SET `status` = 0 , `processes` = 'sp_alicorp_regular_sod_v6';
END IF;
ELSE
        SET @updated_at = (select DATE_SUB(NOW(),INTERVAL 5 HOUR));
INSERT INTO `tempory_processes`(`processes`,`status`,`company_id`,`user_id`,`created_at`,`updated_at`) VALUES ('sp_alicorp_regular_sod_v6',0, _company_id , 0,now(),@updated_at);
END IF;
