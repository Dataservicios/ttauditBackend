...
//effects
    ...
    public function ajaxGetPdvsForPollWithPhotos()
    {
        ...
        //getStoresPollDetails($poll_id,$urlBase,$urlImages,$pregSino,$district="0",$region="0",$ubigeo="0",$rubro="0",$ejecutivo="0",$product_id="0",$company_id="0",$publicity_id="0",$type="0",$cadena="0")
        $datosStores = $this->getStoresPollDetails($poll_id,$this->urlBase,$this->urlImagesFotos,"T","0","0","0","0","0",$product_id,$company_id,$publicity_id,"0","0");//dd($datosStores);

        header('Access-Control-Allow-Origin: *');
        header('Content-type: application/json');
        //return Response::json($datosStores);
        return  Response::json([ 'datos'=> $datosStores,'poll' => $objPoll]);

    }
    public function ajaxGetResultsForAuditWithPhotos($company_id="0",$ajax="0")
    {
        if ($company_id=="0"){
            $valoresPost= Input::all();
            $company_id = $valoresPost['company_id'];
