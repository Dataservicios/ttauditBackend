...
class PollRepo extends BaseRepo{
    ...
    {
        $polls = Poll::join('company_audits','polls.company_audit_id','=','company_audits.id')->select('polls.id','polls.question','polls.sino','polls.options','polls.created_at','company_audits.company_id','company_audits.audit_id','polls.company_audit_id')->where('company_audits.company_id', $company_id)->where('polls.media', 1)->get();
        return $polls;
    }
