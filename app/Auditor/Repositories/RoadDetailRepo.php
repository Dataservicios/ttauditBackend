...
FROM 
  ...
  INNER JOIN `visit_stores` ON (`roads_resume`.`road_id` = `visit_stores`.`road_id`)
where `roads_resume`.`company_id`='".$company_id."' and `roads_resume`.`user_id`='".$user_id."' and  `roads_resume`.`nivel` = 1  and `roads_resume`.`audit`=0 
and `visit_stores`.`company_id` = `roads_resume`.`company_id` 
AND  `visit_stores`.`store_id` = `roads_resume`.`store_id`";

        $consulta=\DB::select($sql);
        return  $consulta;
    }

    public function getStoresAudits($company_id,$audit,$visit_id="0"){
        $roadDetails = RoadDetail::join('stores','road_details.store_id','=','stores.id')->join('roads','road_details.road_id','=','roads.id')->join('visit_stores','visit_stores.road_id','=','roads.id')->select('road_details.id','road_details.company_id','road_details.road_id','road_details.store_id','road_details.created_at','roads.fullname as ruta','roads.user_id','roads.f_ejecucion','roads.created_at as road_created','visit_stores.visit_id','visit_stores.ruteado','visit_stores.created_at as visit_created','stores.fullname as nombre_pdv','stores.type','stores.cadenaRuc','stores.chanel','stores.client','stores.address','stores.district','stores.region','stores.ubigeo','stores.cabecera','stores.latitude','stores.chanel_store_id','stores.longitude','stores.created_at as store_created')->where('road_details.company_id', $company_id)->where('road_details.audit', $audit)->where('visit_stores.visit_id', $visit_id)->groupBy('road_details.store_id')->get();
        return $roadDetails;
    }
