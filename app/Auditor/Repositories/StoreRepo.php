...
class StoreRepo extends BaseRepo {
    ...
    public function getTypeStoreForCampaigne($company_id="0",$tipo="0")
    {
        if ($tipo=="0"){
            if ($company_id=="0"){
        ...
        if($tipo=="1")
        {
            return Store::join('company_stores', 'company_stores.store_id', '=', 'stores.id')->where('company_stores.company_id', $company_id)->where('stores.test', 0)->groupBy('stores.type')->get();
        }
        if($tipo=="2")//transferencista
        {
            return Store::join('company_stores', 'company_stores.store_id', '=', 'stores.id')->where('company_stores.company_id', $company_id)->where('stores.test', 0)->where('chanel_store_id',1)->groupBy('stores.type')->get();
        }
    }
     ...
     * @return array
     */
    public function getHorizontalForCampaigne($company_id, $format="0",$customer_id="0",$study_id="0")
    {
        if ($company_id=="0"){
            $valor = Store::join('company_stores', 'company_stores.store_id', '=', 'stores.id')->join('companies','company_stores.company_id', '=', 'companies.id')->where('companies.active', 1)->where('companies.customer_id', $customer_id)->where('companies.study_id', $study_id)->where('stores.test', 0)->where('stores.type','<>', 'CADENA')->groupBy('stores.type')->get();
        }else{
     ...
     * @return array
     */
    public function getCadenasForCampaigne($company_id, $format="0",$customer_id="0",$study_id="0")
    {
        if ($company_id=="0"){
            $valor = Store::join('company_stores', 'company_stores.store_id', '=', 'stores.id')->join('companies','company_stores.company_id', '=', 'companies.id')->where('companies.active', 1)->where('companies.customer_id', $customer_id)->where('companies.study_id', $study_id)->where('stores.test', 0)->where('stores.type', 'CADENA')->groupBy('stores.cadenaRuc')->get();
        }else{
...
ORDER BY `a`.`created_at` DESC";
    ...
    public function getCadenaRucForType($company_id,$type,$customer_id="0",$study_id="0")
    {
        if ($company_id=="0")
        {
            return Store::join('company_stores', 'company_stores.store_id', '=', 'stores.id')->where('companies.customer_id', $customer_id)->where('companies.study_id', $study_id)->where('stores.test', 0)->type($type)->groupBy('stores.cadenaRuc')->get();
        }else{
            return Store::join('company_stores', 'company_stores.store_id', '=', 'stores.id')->where('company_stores.company_id', $company_id)->where('stores.test', 0)->type($type)->groupBy('stores.cadenaRuc')->get();
        }

    }


    public function getStoreForCompanyCod($company_id,$search)
    {
        $store = Store::join('company_stores','company_stores.store_id','=','stores.id')->select('stores.id','stores.fullname','stores.address','stores.district','stores.region','stores.ubigeo','stores.codclient')->where('company_stores.company_id', $company_id)->where('stores.codclient',$search)->first();
        return $store;
    }
