...
class PollDetailRepo extends BaseRepo{
    ...
    public function searchPollDetail($pollDetail,$tipo="0")
    {
        ...
        if ($tipo == "4"){
            $result_poll_detail = PollDetail::where('poll_id', $pollDetail->poll_id)->where('store_id',$pollDetail->store_id)->where('company_id',$pollDetail->company_id)->where('product_id',$pollDetail->product_id)->where('publicity_id',$pollDetail->publicity_id)->where('stock_product_pop_id',$pollDetail->stock_product_pop_id)->where('visit_id',$pollDetail->visit_id)->where('laboratory_id',$pollDetail->laboratory_id)->first();
        }
...
GROUP BY a.store_id";
                    ...
                    if (count($storesDetail)>0){
                        foreach ($storesDetail as $stores){
                            $detailResult[] = \DB::table('poll_details')->join('stores','poll_details.store_id','=','stores.id')->select('stores.id', 'stores.cadenaRuc','stores.type','stores.codclient','stores.fullname','stores.ubigeo','stores.region','stores.district','stores.address','poll_details.comentario','poll_details.created_at')->where('stores.id', $stores)->get();
                        }
                    }else{
                        $detailResult =0;
                    }
...
GROUP BY a.store_id";
    ...
    /**
     * Permite saber la cantidad de puntos que responden una pregunta de tipo Si/No y de opciones
     * @param $company_id
     * @param $poll_id
     * @param $visit_id
     * @param $result
     * @param string $publicity_id
     * @param string $product_id
     * @param string $typeStore
     * @param string $cadena
     * @param string $ubigeo
     * @return mixed
     */
    public function detailsResponseSiNo($company_id, $poll_id, $visit_id, $result, $publicity_id="0", $product_id="0", $typeStore="0", $cadena="0", $ubigeo="0",$store_id="0",$district="0",$region="0",$ejecutivo="0",$rubro="0")
    {//dd($company_id, $poll_id, $visit_id, $result, $publicity_id, $product_id, $typeStore, $cadena, $ubigeo,$store_id,$district,$region,$ejecutivo,$rubro);
        $pollDetails =  PollDetail::join('stores','poll_details.store_id','=','stores.id')->select('poll_details.id','poll_details.store_id','poll_details.poll_id','poll_details.sino','poll_details.options','poll_details.limits','poll_details.media','poll_details.coment','poll_details.result','poll_details.limite','poll_details.comentario','poll_details.auditor','poll_details.product_id','poll_details.publicity_id','poll_details.company_id','poll_details.category_product_id','poll_details.visit_id','poll_details.stock_product_pop_id','poll_details.comentOptions','poll_details.created_at','poll_details.updated_at','stores.cadenaRuc','stores.fullname','stores.type','stores.tipo_bodega','stores.chanel','stores.client','stores.DNI','stores.owner','stores.client','stores.address','stores.district','stores.region','stores.ubigeo','stores.distributor','stores.codclient','stores.latitude','stores.longitude','stores.nomb_ejecutivo','stores.ejecutivo','stores.coordinador','stores.gerzonal','stores.cabecera')
            ->campaigne($company_id)->pollId($poll_id)->visitId($visit_id)->result($result)->publicityId($publicity_id)->productId($product_id)->storeId($store_id)->typeStore($typeStore)->cadenaRuc($cadena)->ubigeoStore($ubigeo)->districtStore($district)->regionStore($region)->ejecutivoStore($ejecutivo)->rubroStore($rubro)->groupBy('poll_details.store_id')->get();
        //dd(count($pollDetails));
        return $pollDetails;
    }

    /**
     * Permite saber la cantidad de puntos que responden una pregunta de tipo Si/No y de opciones
     * pero que hayan sido cerrados en road_details
     * @param $company_id
     * @param $poll_id
     * @param $visit_id
     * @param $result
     ...
     * @return mixed
     */
    public function detailsResponseSiNoAudits($company_id, $poll_id, $visit_id, $result, $publicity_id="0", $product_id="0", $typeStore="0", $cadena="0", $ubigeo="0",$store_id="0",$district="0",$region="0",$ejecutivo="0",$rubro="0")
    {
        $total ="SELECT
  `poll_details`.`id`,
  `poll_details`.`poll_id`,
...
INNER JOIN (select store_id from road_details where company_id=".$company_id."  and audit=1 group by store_id) a ON (`a`.`store_id` = `poll_details`.`store_id`)
WHERE
  `stores`.`type` = '".$typeStore."' AND 
  `poll_details`.`company_id` = ".$company_id." AND
  `poll_details`.`result` = ".$result." AND
  `poll_details`.`poll_id` = ".$poll_id." and
`poll_details`.`visit_id` = ".$visit_id." group by `poll_details`.`store_id`";
        $pollDetails=\DB::select($total);
        return $pollDetails;
    }

    /**
     * Permite obtener todos los registros por punto para una pregunta de tipo solo comentario
     * @param $company_id
     * @param $poll_id
     * @param string $publicity_id
     * @param string $product_id
     ...
     * @return mixed
     */
    public function detailsResponseComment($company_id, $poll_id, $publicity_id="0", $product_id="0", $typeStore="0", $cadena="0", $ubigeo="0",$category_product_id="0")
    {//dd($company_id,$poll_id,$publicity_id,$product_id, $typeStore, $cadena, $ubigeo,$category_product_id);
        $pollDetails =  PollDetail::join('stores','poll_details.store_id','=','stores.id')->campaigne($company_id)->pollId($poll_id)->publicityId($publicity_id)->productId($product_id)->typeStore($typeStore)->categoryProductId($category_product_id)->cadenaRuc($cadena)->ubigeoStore($ubigeo)->orderBy('poll_details.product_id','ASC')->get();
        return $pollDetails;
    }

    public function detailsGroupStores($company_id,$poll_id,$visit_id,$publicity_id="0",$product_id="0",$typeStore="0",$cadena="0",$ubigeo="0")
    {
        $pollDetails =  PollDetail::join('stores','poll_details.store_id','=','stores.id')->campaigne($company_id)->pollId($poll_id)->visitId($visit_id)->publicityId($publicity_id)->productId($product_id)->typeStore($typeStore)->cadenaRuc($cadena)->groupBy('store_id')->get();
        return $pollDetails;
    }

    /**
     * @param $company_id, $store_id
     * Retorna la cantidad de visitas registradas a cada tienda
     */
    public function getVisitStores($company_id,$publicity_id="0",$store_id="0",$typeStore="0",$cadena="0")
    {
        $pollDetails =  PollDetail::join('stores','poll_details.store_id','=','stores.id')->campaigne($company_id)->storeId($store_id)->publicityId($publicity_id)->typeStore($typeStore)->cadenaRuc($cadena)->groupBy('poll_details.visit_id')->get();
        return $pollDetails;
    }

    /**
     * @param $company_id, $store_id
     * Retorna la ultima visita registrada en los datos ingresados
     */
    public function getEndVisitStores($company_id,$publicity_id="0",$store_id="0",$typeStore="0",$cadena="0")
    {
        $pollDetails =  PollDetail::join('stores','poll_details.store_id','=','stores.id')->campaigne($company_id)->storeId($store_id)->publicityId($publicity_id)->typeStore($typeStore)->cadenaRuc($cadena)->groupBy('poll_details.visit_id')->orderBy('poll_details.visit_id','DESC')->first();
        return $pollDetails;
    }
