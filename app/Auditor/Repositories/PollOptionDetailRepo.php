    ...
    public function getTotalOptionForAll($group_poll_id="0",$ejecutivo="0",$product_id="0",$cadena="0",$poll_id="0",$limit="0",$horizontal="0",$ubigeo="0")
    {
        ...
        $total1 ="SELECT
                ...
                FROM
                  ...
                  INNER JOIN `polls` ON (`poll_options`.`poll_id` = `polls`.`id`)".$where;
...
GROUP BY
  ...
  `poll_option_details`.`poll_option_id`";
    ...
    /**
     * Obtiene los registros por opción
     * @param $company_id
     ...
     * @return mixed
     */
    public function getResultOption($company_id, $poll_option_id, $visit_id, $publicity_id="0", $product_id="0", $typeStore="0", $cadena="0", $ubigeo="0", $ejecutivo="0")
    {
        $pollDetails =  PollOptionDetail::join('stores','poll_option_details.store_id','=','stores.id')->campaigne($company_id)->pollOptionId($poll_option_id)->visitId($visit_id)->publicityId($publicity_id)->productId($product_id)->typeStore($typeStore)->cadenaRuc($cadena)->joinUbigeo($ubigeo)->joinEjecutivo($ejecutivo)->groupBy('store_id')->get();
        return $pollDetails;
    }


    public function getTotalForOptions($poll_id="0",$visit_id="0",$publicity_id="0",$product_id="0",$typeStore="0",$cadena="0",$ubigeo="0",$ejecutivo="0",$group="0")
    {
        $pollDetails =  PollOptionDetail::join('stores','poll_option_details.store_id','=','stores.id')->join('poll_options','poll_option_details.poll_option_id','=','poll_options.id')->join('polls','poll_options.poll_id','=','polls.id')->select(\DB::raw('count(*) as nro, poll_options.options, poll_options.id'))->joinPollId($poll_id)->visitId($visit_id)->publicityId($publicity_id)->joinOptionProductId($product_id)->typeStore($typeStore)->cadenaRuc($cadena)->joinUbigeo($ubigeo)->joinEjecutivo($ejecutivo)->joinGroupColumn($group)->orderBy('nro','desc')->get();
        return $pollDetails;
    }
