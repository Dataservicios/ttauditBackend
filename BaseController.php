...
class BaseController extends Controller {
	...
	public function getQuestionsSendEmail()
	{
        ...
        $this->questionSendEmail[] = array('company_id' => 223,'poll_id' => 3836,'send' =>1,'result' =>0, 'poll_option_id' => 0,'mask' =>'ttauditRaul]sistemas');
        $this->questionSendEmail[] = array('company_id' => 250,'poll_id' => 4389,'send' =>1,'result' =>0, 'poll_option_id' => 0,'mask' =>'sistemas');
        $this->questionSendEmail[] = array('company_id' => 250,'poll_id' => 4406,'send' =>1,'result' =>0, 'poll_option_id' => 0,'mask' =>'sistemas');
        $this->questionSendEmail[] = array('company_id' => 250,'poll_id' => 4409,'send' =>1,'result' =>1, 'poll_option_id' => 0,'mask' =>'sistemas');
	...
	public function getGroupsEmails()
	{
        ...
        $this->groupsEmail['ttauditIBK'] = array('email' => 'ellerena@ttaudit.com:Elba|jorgepretell@ttaudit.com:Jorge');
	...
	public function sendEmails($store_id,$textoContent,$motivo,$auditor,$comentario,$cliente,$tipo_bodega,$agente,$dir,$address,$district,$foto,$fechaHoraEnvio,$emails,$fono="0")
	{
		...
		if (($fono<>"0") and ($fono<>"M") and ($fono<>"Alerts")){
			$datai = ['origen' => $store_id,
				'tipo'  =>  'LocalCerrado',
				'titulo'=> $textoContent,
				'motivo' => $motivo,
	...
	public function getValores()
	{
        ...
        $this->valoresCampaigne[162] = array('ventanaW' => 2673,'abierto' => 2671,'permitio' =>2672,'existeVent' => 2674, 'visibleVent' =>2675, 'comoEstaVent'=>2676,'encontroExhi'=>2677);
	...
	public function generateMenusBayer($company_id,$audit_id, $cat="0")
	{
        ...
        if (($audit_id == 0) and ($cat=="10")){
            $menus[] = array('nombre' => "Histórico de Competencias", 'url' => route('lastBrandHistory'), 'active' => 1,'icon' => 'audit', 'submenu1' => $submenu1);
        }else{
            $menus[] = array('nombre' => "Histórico de Competencias", 'url' => route('lastBrandHistory'), 'active' => 0,'icon' => 'audit', 'submenu1' => $submenu1);
        }

        if (($audit_id == 0) and ($cat=="11")){
            $menus[] = array('nombre' => "Histórico Visibilidad", 'url' => route('visibilityHistory'), 'active' => 1,'icon' => 'audit', 'submenu1' => $submenu1);
        }else{
            $menus[] = array('nombre' => "Histórico Visibilidad", 'url' => route('visibilityHistory'), 'active' => 0,'icon' => 'audit', 'submenu1' => $submenu1);
        }*/

        if (($audit_id == 0) and ($cat=="7"))
        {
            $submenu1[0] = array('nombre' => "Marcas Moderno", 'url' => route('detailBrandHistory', array('Moderno')), 'active' => 0,'icon' => 'ok');
            $submenu1[1] = array('nombre' => "Marcas Tradicional", 'url' => route('detailBrandHistory', array('Tradicional')), 'active' => 0,'icon' => 'ok');
            //$menus[] = array('nombre' => "Histórico por Marcas", 'url' => route('homeBrandHistory'), 'active' => 1,'icon' => 'menos', 'submenu1' => $submenu1);
        }else{
            if (($audit_id == 0) and (($cat=="8") or ($cat == "9"))){
                if ($cat=="8"){
                    $submenu1[0] = array('nombre' => "Marcas Moderno", 'url' => route('detailBrandHistory', array('Moderno')), 'active' => 1,'icon' => 'ok');
                }else{
                    $submenu1[0] = array('nombre' => "Marcas Moderno", 'url' => route('detailBrandHistory', array('Moderno')), 'active' => 0,'icon' => 'ok');
                }
                if ($cat=="9"){
                    $submenu1[1] = array('nombre' => "Marcas Tradicional", 'url' => route('detailBrandHistory', array('Tradicional')), 'active' => 1,'icon' => 'ok');
                }else{
                    $submenu1[1] = array('nombre' => "Marcas Tradicional", 'url' => route('detailBrandHistory', array('Tradicional')), 'active' => 0,'icon' => 'ok');
                }
                //$menus[] = array('nombre' => "Histórico por Marcas", 'url' => route('homeBrandHistory'), 'active' => 1,'icon' => 'menos', 'submenu1' => $submenu1);
            }
            //$menus[] = array('nombre' => "Histórico por Marcas", 'url' => route('homeBrandHistory'), 'active' => 0,'icon' => 'mas', 'submenu1' => $submenu1);
        }
...
//dd($menus);
	...
	public function generateMenusInterbank($company_id,$audit_id, $cat="0")
	{
		...
		if ($audit_id==-1){
            $menus[] = array('nombre' => "Inventario Tiendas", 'url' => route('getPhotosInventory', array(139)), 'active' => 1,'icon' => 'audit');
        }else{
            $menus[] = array('nombre' => "Inventario Tiendas", 'url' => route('getPhotosInventory', array(139)), 'active' => 0,'icon' => 'audit');
        }
...
//dd(count($stores),$soloPhotos,$poll_option_id);
	...
	public function getDetailPublicitiesAlicorp($publicity_id,$contaminated,$urlBase,$urlImages,$valores,$company_id)
	{
		...
		if(count($stores)>0){
			foreach ($stores as $store){
				...
				$datosStores[] = array('store_id' => $store->id, 'tipo' => $store->type, 'tipo_bodega' => $store->tipo_bodega,'cadenaRuc' =>$store->cadenaRuc, 'fullname' => $store->fullname, 'direccion' => $store->address, 'departamento' => $store->ubigeo, 'Provincia' => $store->region, 'distrito' => $store->district,'dex' => $store->distributor, 'arrayFoto' => $datosFoto, 'fecha' => $store->created_at,'publicity_details_id'=>$store->publicity_details_id);
				//dd($datosStores);
				unset($datosFoto);
			}
			//dd($datosStores);
    ...
    /**
     * Para obtener todos los poll_id que solo se usaran para el sistema web
     * @param $customer_id
     * @param $estudio
     * @return array
     */
    public function getAllPollsWeb($customer_id, $estudio)
    {
        ...
        if (count($companies)>0)
        {
            ...
            {
                $c=$c+1;
                $objPolls = $this->pollRepo->getQuestionForWeb($company_data->id);
    ...
    public function pricesAverageAndFashion($regs)
    {
        if (count($regs)>0){
            $products = $regs->groupBy(function($item){ return $item->product_id; });
            //dd($products);
            $cantPdvPorProdClient=0;
            $success=1;
            foreach ($products as $index => $reg) {
                $regColl = Collection::make($reg);
                $porClientes =  $regColl->groupBy(function($item){ return $item->cadenaRuc; });
                $producto = $this->productRepo->find($index);
                $product0 = array('tipo'=>$producto->fullname);$cliente0 =[];$moda0 =[];
                foreach ($porClientes as $porClienteNombre => $porCliente) {

                    $rege=Collection::make($porCliente);
                    foreach ($rege as $item) {
                        //dato por cliente y producto
                        $precios[] = array('store_id'=>$item->store_id,'precio'=>floatval($item->comentario),'poll_details_id'=>$item->id);

                    }
                    $groupPrecios =  $rege->groupBy(function($item){ return $item->comentario; });
                    foreach ($groupPrecios as $indexPrecio => $groupPrecio) {
                        $moda1 = array($porClienteNombre=>$indexPrecio);
                        $moda0 = array_merge($moda0,$moda1);
                        $detallesModa[] = array('producto'=>$producto->fullname,'cliente'=>$porClienteNombre,'precios'=>$groupPrecio,'moda1'=>$indexPrecio,'cantPreciosPorModa1'=>count($groupPrecio));
                        break;
                    }


                    $cant = $rege->count();//por cliente y producto
                    $cantPdvPorProdClient = $cantPdvPorProdClient + $cant;
                    $totalVolume = $rege->sum(function ($re) {
                        return $re->comentario;
                    });
                    $promedio = round($totalVolume/$cant,2);

                    $cliente1 = array($porClienteNombre=>$promedio);

                    $cliente0 = array_merge($cliente0,$cliente1);

                    $detalles[] = array('producto'=>$producto->fullname,'cliente'=>$porClienteNombre,'precios'=>$precios,'promedio'=>$promedio,'cantPreciosPorCliente'=>$cant);
                }
                $valoresPop[] = array_merge($product0,$cliente0);
                $valoresModa[] = array_merge($product0,$moda0);
            }
            $valores = array('valoresPop'=>$valoresPop,'detalles'=>$detalles,'valoresModa'=>$valoresModa,'detallesModa'=>$detallesModa);
        }else{
            $valoresPop = [];$detalles=[];$valoresModa=[];$detallesModa=[];
            $valores = array('valoresPop'=>$valoresPop,'detalles'=>$detalles,'valoresModa'=>$valoresModa,'detallesModa'=>$detallesModa);
        }
    ...
    public function getResponsePollsNew($store_id,$company_id,$publicity_id,$poll_id,$type,$product_id="0")
    {
        if ($type == 'YesNo')
        {
            if ($publicity_id==0){
            ...
            if ($cantReg>0)
            {
                ...
                $responses = array('texto' => $texto,'pollDetails' => $storeOpen);
            }else{
                $responses = array('texto' => "No hay ingreso",'pollDetails' => []);
            }
        }
        if ($type == 'Option')
        {
            ...
            if ($cantReg>0)
            {
                ...
                $responses = array('texto' => $texto,'pollDetails' => $storeComoEsta,'pollOptionsDetails' => $responseOption, 'options' =>$options);
            }else{
                $responses = array('texto' => 'No hay registros','pollDetails' => [],'pollOptionsDetails' => [], 'options' =>[] );
            }
        }
        return $responses;
    }
        ...
        if (count($store_ids) > 0) {
            foreach ($store_ids as $index => $poll_detail){//dd($poll_detail);
                ...
                if ($objPoll->sino==1){
                    if ($cantReg>0)
                    {
                        if ($cantReg>1){
                            ...
                            if ($poll_detail[0]->result==1){
                                $texto = "Sí (".$poll_detail[0]->created_at.")";
                            }else{
                ...
                if ($objPoll->options==1){
                    ...
                    if ($cantReg>0)
                    {
                        $responseOption = $this->PollOptionDetailRepo->getResponseOptionPublicity($poll_detail[0]->store_id,$poll_id,$publicity_id,$company_id);//dd($responseOption);
                        $cantReg1 = count($responseOption);
                        if ($cantReg1>0)
                        {
                            $texto = "Opciones seleccionadas"."<br>";
                            $texto .= "<ul>";
                            foreach ($responseOption as $option)
                            {
                                ...
                                $id = $option->id;
                                $poll_option_details_id = $option->poll_option_details_id;
                                $optionSelected[] = array('poll_option_detail_id'=>$poll_option_details_id,'poll_option_id'=>$id);
                            }
                            $texto .= "</ul>";
                        }else{
                            $texto = "No Hay Opciones ingresadas ";$optionSelected=[];
                        }
                        $responseOptions = array('texto' => $texto,'pollOptionsDetails' => $responseOption, 'options' =>$options,'optionSelected'=>$optionSelected);unset($optionSelected);
                    }else{
                        $responseOptions = array('texto' => 'No hay registros','pollOptionsDetails' => [], 'options' =>$options,'optionSelected'=>[]);
                    }
                }else{
                    $responseOptions = array('texto' => 'No es de tipo options','pollOptionsDetails' => [], 'options' =>[],'optionSelected'=>[]);
                }
                $datosStores[] = array('reponseOption'=>$responseOptions,'responseSiNo'=>$responsesSiNo,'store' =>$objStore, 'arrayFoto' => $datosFoto, 'created_at' => $poll_detail[0]->created_at,'updated_at' => $poll_detail[0]->updated_at);
                unset($datosFoto);unset($responseOptions);unset($responsesSiNo);unset($objStore);
            }
        }else{
            $datosStores=[];
        }
